...
function showPackets(div, packets) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    var p = packets[i];
    row.innerHTML = '<input class="sc" type="text" value="' + toHexStr(p.sc) + '"></span> ' + '<input class="data mono" type="text" value="' + toHexStr(p.data) + '"></span>' + '<input class="hv mono" type="text" value="' + toHexStr(p.hv) + '"></span>';
...
function showHV(rows) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    data = rows[i];
    row.innerHTML = '<input class="hv mono" type="text" value="' + toHexStr(data.received) + '"></span> ' + '<input class="hv mono" type="text" value="' + toHexStr(data.calculated) + '"></span>' + '<input class="match" type="text" value="' + toIcon(data.isMatch) + '"></span>';
    row.className = 'row fadeable';
    div.appendChild(row)
  }
  var children = div.children;
