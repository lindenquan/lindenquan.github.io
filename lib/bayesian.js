...
var JoinTree = function () {
    ...
    _createClass(JoinTree, [{
        ...
        key: "sortLeaves",
        value: function sortLeaves(leaves, root) {
            var _this3 = this;

            var array = Array.from(leaves);
            if (DEBUG) {
                console.log("before sort:");
                array.forEach(function (v) {
                    return console.log(v.distr.name);
                });
            }
        ...
        value: function runShaferShenoy() {
            ...
            var _loop3 = function _loop3() {
                ...
                leaves.forEach(function (leaf) {
                    var father = leaf.father;
                    leaf.setShaferMessage(father, _this4.findShaferSeperator(leaf, father), _this4.findAllShaferSeperators(leaf));
                    if (father != root) {
                        newLeaves.add(father);
                    }
                });
                leaves = newLeaves;
                _this4.sortLeaves(leaves, root);
            };
