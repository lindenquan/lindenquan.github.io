...
(function($) {
        ...
        $(this).each(function() {
            ...
            if (typeof options == 'string') {
                ...
                if ($(this).data('booklet')) {
                    ...
                    if (command == 'next') { obj.next() } else if (command == 'prev') { obj.prev() }
    ...
    function booklet(target, options, id) {
        ...
        target.data('id', id);
        target.data('total', src.children().length);
            ...
            if (opts.pageSelector) {
                ...
                for (i = 0; i < opts.pTotal; i += 2) {
                    ...
                    if (opts.closed) {
                        ...
                        j--;
                        if (i == 0) { nums = '1' } else if (i == opts.pTotal - 2) { nums = opts.pTotal - 2 } else { nums = (j + 1) + '-' + (j + 2); }
                        //nums for closed book with covers
                        if (opts.covers) {
                            j--;
                            if (i == 0) { nums = '' } else if (i == opts.pTotal - 2) { nums = '' } else { nums = (j + 1) + '-' + (j + 2); }
                        }
                    }
                    ...
                    if (opts.direction == 'RTL') {
                        ...
                        if (opts.closed) {
                            if (i == opts.pTotal - 2) { nums = '1' } else if (i == 0) { nums = opts.pTotal - 2 } else { nums = (Math.abs(j - opts.pTotal) - 3) + ' - ' + ((Math.abs(j - opts.pTotal) - 2)); }

                            if (opts.covers) {
                                if (i == opts.pTotal - 2) { nums = '' } else if (i == 0) { nums = '' } else { nums = (Math.abs(j - opts.pTotal) - 5) + ' - ' + ((Math.abs(j - opts.pTotal) - 4)); }
                            }
                        }
        ...
        if (opts.keyboard) {
            ...
            $(document).keyup(function(event) {
                if (event.keyCode == 37) { self.prev(); } else if (event.keyCode == 39) { self.next(); }
            });
        }
        ...
        if (opts.hash) {
            setupHash();
            clearInterval(mybookletInterval);
        ...
        function updatePager() {
            if (opts.pageSelector) {
                if (opts.direction == 'RTL') {
                    ...
                    if (opts.closed) {
                        if (opts.curr == opts.pTotal - 2) { nums = '1' } else if (opts.curr == 0) { nums = opts.pTotal - 2 } else { nums = (Math.abs(opts.curr - opts.pTotal) - 2) + ' - ' + ((Math.abs(opts.curr - opts.pTotal) - 1)); }

                        if (opts.covers) {
                            if (opts.curr == opts.pTotal - 2) { nums = '' } else if (opts.curr == 0) { nums = '' } else { nums = (Math.abs(opts.curr - opts.pTotal) - 3) + ' - ' + ((Math.abs(opts.curr - opts.pTotal) - 2)); }
                        }
                    }
                    ...
                    if (opts.closed) {
                        if (opts.curr == 0) { nums = '1' } else if (opts.curr == opts.pTotal - 2) { nums = opts.pTotal - 2 } else { nums = (opts.curr) + '-' + (opts.curr + 1); }

                        if (opts.covers) {
                            if (opts.curr == 0) { nums = '' } else if (opts.curr == opts.pTotal - 2) { nums = '' } else { nums = (opts.curr - 1) + '-' + (opts.curr); }
                        }
                    }
                    $(opts.menu + ' .b-selector-page .b-current').text(nums);
                }
